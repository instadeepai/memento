...
RUN micromamba create -y --file /tmp/environment.yml \
    ...
    # cannot install chex 0.1.7 with micromamba because of jumanji, so we use pip
    && micromamba run -n popcorn2 pip install chex==0.1.7
...
ENV PATH=/opt/conda/envs/popcorn2/bin/:$PATH APP_FOLDER=/app
...
ENV USER=eng
ENV GROUP=eng
ENV LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/opt/conda/envs/popcorn2/lib
