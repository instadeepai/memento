...
from memento.utils.devices import run_on_master


class module_unpickler(pickle.Unpickler):
    def find_class(self, module, name):
        if module == "poppy.trainers.trainer":
            module = "memento.trainers.trainer"

        if module == "poppy.memory.external_memory":
            module = "memento.memory.external_memory"

        if module == "poppy.memory.types":
            module = "memento.memory.types"

        return super().find_class(module, name)


def robust_load(file_path):
    return module_unpickler(file_path).load()
...
def load_memory_decoder_checkpoint(cfg):

    saved_decoder = None

    if cfg.checkpointing.restore_memory_path:
        cfg.checkpointing.memory_checkpoint_fname_load = os.path.splitext(
            cfg.checkpointing.memory_checkpoint_fname_load
        )[0]
        with open(
            os.path.join(
                cfg.checkpointing.restore_memory_path,
                cfg.checkpointing.memory_checkpoint_fname_load + ".pkl",
            ),
            "rb",
        ) as f:
            saved_state = pickle.load(f)
            _saved_encoder, saved_decoder = hk.data_structures.partition(
                lambda m, n, p: "encoder" in m, saved_state.params
            )

    return saved_decoder
