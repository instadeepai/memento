...
from memento.environments.jobshop.types import Observation, State

from jumanji.types import termination, transition, restart

from memento.environments.poppy_env import PoppyEnv
from memento.environments.jobshop.utils import generate_problem


class PoppyJobShop(JobShop, PoppyEnv):

    def __init__(self, num_jobs=10, num_machines=10, max_op_duration=99):
    ...
    def get_episode_horizon(self) -> int:
        return int(jnp.sum(self.durations) // (self.num_machines / 4))
    ...
    def reset_from_state(self, problem, start_position):
    ...
    def get_problem_size(self) -> int:
        return self.num_machines * self.num_jobs
