...
from memento.environments.tsp.utils import (
    compute_tour_length,
...
from memento.environments.cvrp.types import Observation
from memento.environments.cvrp.utils import (
    DEPOT_IDX,
    MIN_NORM_FACTOR,
    generate_problem,
    get_augmentations,
...
class PoppyCVRP(Environment[State], PoppyEnv):
    ...
    def reset_from_state(
        ...
        state = State(
            ...
            visited_mask=jnp.zeros(self.num_nodes + 1, dtype=jnp.int8)
            .at[DEPOT_IDX]
            .set(1),
    ...
    def observation_spec(self) -> specs.Spec[Observation]:
        ...
        num_visited = specs.DiscreteArray(
            self.num_nodes + 1, dtype=jnp.int32, name="num_visited"
        )
    ...
    def _state_to_observation(self, state: State) -> Observation:
        ...
        demands = state.demands.at[...].set(
            jnp.float32(state.demands / self.norm_factor)
        )
        problem = jnp.hstack([state.coordinates, demands[:, None]])
        num_visited = state.visited_mask.sum()
        ...
        return Observation(
            ...
            is_done=(num_visited == self.num_nodes + 1).astype(int),
        )
